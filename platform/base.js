...
var windowDefaultEncoding = 'cp936';
  ...
  speak(text, voice, speed, callback) {
    ...
    if (pipedData) {
      if (process.platform !== 'win32') {
        ...
        let encodePipedData = iconv.encode(pipedData, windowDefaultEncoding);
        this.child.stdin.end(encodePipedData);
      }
    }
    ...
    this.child.stderr.once('data', (data) => {
      if (process.platform !== 'win32') {
        ...
        let errorMessage = iconv.decode(data, windowDefaultEncoding);
